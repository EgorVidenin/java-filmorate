...
public class FilmDbStorage {
    ...
    public Film create(Film film) {
        ...
        if (film.getMpa() != null) {
            ...
            checkValidateIdExistence(ratingId, "ratings");
        ...
        if (!genres.isEmpty()) {
            film.getGenres().forEach(genre -> checkValidateIdExistence(genre.getId(), "genres"));
    ...
    public Film update(Film film) {
        checkExistsById(film.getId(), "films");
        template.update(
        ...
        if (film.getMpa() != null) {
            checkExistsById(film.getMpa().getId(), "ratings");
            template.update(
                    "update films set rating_id = ? where id = ?",
    ...
    public Film addLike(Integer filmId, Integer userId) {
        checkExistsById(filmId, "films");
        checkExistsById(userId, "users");
    ...
    private void checkExistsById(int id, String tableName) {
    ...
    private void checkValidateIdExistence(int id, String tableName) {
        String select = "select exists (select id as match from " + tableName + " where id = ?) as match";
        if (Boolean.FALSE.equals(template.queryForObject(select,
                (rs, rowNum) -> rs.getBoolean("match"), id))) {
